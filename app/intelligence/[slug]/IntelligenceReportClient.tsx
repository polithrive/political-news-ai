...
type IntelligenceReportClientProps = {
  slug: string;
  articleUrl: string | null;
  parseError: "missing" | "malformed" | null;
};

export default function IntelligenceReportClient({
  slug,
  articleUrl,
  parseError,
}: IntelligenceReportClientProps) {
  ...
  useLayoutEffect(() => {
    ...
    async function resolveArticle(): Promise<Article | null> {
      if (parseError === "malformed") {
        return null;
      }
      ...
      if (articleUrl) {
        ...
        if (
          selectedArticle &&
          articleUrlsMatch(selectedArticle.url, articleUrl)
        ) {
          return selectedArticle;
        }

        try {
          const feedArticles = await getLatestNews();
          const matched = feedArticles.find((candidate) =>
            articleUrlsMatch(candidate.url, articleUrl)
          );
          ...
          // NewsAPI/feed lookup is optional; fall through to URL analysis.
        }

        return createUrlSubmittedArticle(articleUrl);
      }
      ...
      if (
        selectedArticle &&
        intelligenceSlugMatchesTitle(slug, selectedArticle.title)
      ) {
        return selectedArticle;
      }
    ...
    async function initializeIntelligence() {
      const selectedArticle = await resolveArticle();
      ...
      saveSelectedArticle(selectedArticle);
  ...
  useEffect(() => {
    ...
    const canonicalPath = createIntelligenceHref(article);
  ...
  if (!article) {
    const unresolvedTitle =
      parseError === "malformed"
        ? "This story link is not valid."
        : "This story could not be opened from the link.";

    const unresolvedBody =
      parseError === "malformed"
        ? "The link does not contain a usable article address. Open a story from Today, or paste an article URL on the homepage."
        : "The Angle Report needs the story address in the link to load this brief in a new browser. Open a 60-second brief from Today, or paste the original article URL.";
